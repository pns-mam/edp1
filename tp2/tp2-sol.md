...
$$ \begin{aligned}
  & u_t - \nu\Delta u = f,\quad t > 0,\quad (x,y) \in \Omega,\\
  & u(0,x,y) = u_0(x,y),\quad  (x,y) \in \Omega,
\end{aligned} $$
...
Exécuter le code ci-dessous qui prend en compte :
...
pour calculer la solution stationnaire (Poisson) et déplacer le chauffage pour tester différentes configurations.

```julia
using SparseArrays, Random, LinearAlgebra, Plots
